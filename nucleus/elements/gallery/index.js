...
var Gallery ={
    ...
    getPrevButton: function(){
        if(this.state.active > 0){
            return (
                React.DOM.a({href: "javascript:void(0);", className: "gallery-prev", onClick: this.prev}, "prev")
            )
        }
    },
    getNextButton: function(){
        if(this.state.active < (this.props.slides.length - 1)){
            return (
                React.DOM.a({href: "javascript:void(0);", className: "gallery-next", onClick: this.next}, "next")
            )
        }
    },
    ...
    render: function(){
        ...
        var slides = this.props.slides.map(function(slide, index){
            ...
            return (
                React.DOM.li({className: "monstro-gallery-item" + activeClass, onClick: setActive}, 
                    React.DOM.a({href: "javascript:void(0);"}, 
                        React.DOM.img({src: slide.image})
                    )
                )
            )
        }.bind(this));
        return (
            React.DOM.div({className: "monstro-gallery-wrapper", style: wrapperStyle}, 
                this.getCustomizationDropdown('bottom-middle'), 
                React.DOM.ul({className: "monstro-gallery", style: galleryStyle}, 
                    slides
                ), 
                React.DOM.div({className: "gallery-controls"}, 
                    this.getPrevButton(), 
                    this.getNextButton()
                )
            )
